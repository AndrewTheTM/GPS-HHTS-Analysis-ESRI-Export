...
public class ExportPoints {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String featClassName="f"+args[0].substring(args[0].lastIndexOf("\\")+1, args[0].length()-4);
				...
				for(GPSData g:GPS){
					...
					for(java.lang.reflect.Field fld:g.getClass().getDeclaredFields()){
						if(Modifier.isPublic(fld.getModifiers())){
							if(feat.getFields().findField(fld.getName())>=0){
								try {
									if(fld.getType().toString().toLowerCase().matches("double")){
										feat.setValue(feat.getFields().findField(fld.getName()),fld.getDouble(g));
									}else if(fld.getType().toString().toLowerCase().matches("int")){
										feat.setValue(feat.getFields().findField(fld.getName()),fld.getInt(g));
									}else if(fld.getType().toString().toLowerCase().contains("date")){
										feat.setValue(feat.getFields().findField(fld.getName()),fld.get(g));
									}else{ 
										feat.setValue(feat.getFields().findField(fld.getName()),fld.get(g).toString());
									} 
					...
					feat.store();
					if(counter % Math.round((GPS.size()*0.1))==0 || counter==GPS.size())
						System.out.println(Math.round(((double) counter/GPS.size())*100)+"% complete");
					counter++;
				}
			}else{
